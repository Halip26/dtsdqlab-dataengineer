...
top_brands = (
    dataset[dataset["order_month"] == "2019-12"]
...
dataset_top5brand_dec_per_product = (
    dataset_top5brand_dec.groupby(["brand", "product_id"])["quantity"]
    .sum()
    .reset_index()
)
...
s_sort = (
    dataset_top5brand_dec_per_product.groupby("brand")["product_id"]
    .nunique()
...
dataset_top5brand_dec_per_product.groupby(["brand", "quantity_group"])[
    "product_id"
].nunique().reindex(index=s_sort.index, level="brand").unstack().plot(
    kind="bar", stacked=True
)
plt.title(
    "Number of Sold Products per Brand, December 2019",
    loc="center",
    pad=30,
    fontsize=15,
    color="blue",
